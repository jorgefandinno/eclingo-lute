...
def solve(lib, reified_program):
    reified_program = parse_program(lib, reified_program)
    config = _eclingo.config.AppConfig()
    config.eclingo_semantics = "c19-1"

    solver = SolverReification(lib, reified_program, config)
...
def solve_with_config(lib, reified_program, **kwargs):
    reified_program = parse_program(lib, reified_program)
    config = _eclingo.config.AppConfig()
    config.eclingo_semantics = "c19-1"
...
class TestCase(unittest.TestCase):
    ...
    def assert_models(self, models, expected):
        def canonical(wviews):
            return sorted(
                (WorldView(sorted(w.symbols, key=str)) for w in wviews), key=str
            )

        self.assertEqual(canonical(models), canonical(expected))
        ...
        self.assert_models(
            ...
            [
                WorldView(
                    [
                        EpistemicLiteral(
                            Function(self.lib, "a", [], True), Sign.NoSign, False
                        ),
                        EpistemicLiteral(
                            Function(self.lib, "b", [], True), Sign.NoSign, False
                        ),
...
class TestIgnoreShows(TestCase):
    def test_ignore_shows_uses_base_builder(self):
        ...
        self.assert_models(
            solve_with_config(self.lib, _PRG_A_KNOWS_A, ignore_shows=True),
            [
                WorldView(
                    [EpistemicLiteral(Function(self.lib, "a", [], True), 0, False)]
                )
...
class TestPreprocessing(TestCase):
    def test_preprocessing_satisfiable(self):
        ...
        self.assert_models(
            solve_with_config(self.lib, _PRG_A_KNOWS_A, preprocessing_level=1),
            [
                WorldView(
                    [EpistemicLiteral(Function(self.lib, "a", [], True), 0, False)]
                )
    ...
    def test_preprocessing_unsatisfiable_returns_early(self):
        ...
        self.assert_models(
            solve_with_config(self.lib, _PRG_UNSAT, preprocessing_level=1),
            [],
        )
